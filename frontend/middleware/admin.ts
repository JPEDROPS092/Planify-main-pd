...
function hasRequiredPermissions(
  ...
  return requiredPermissions.every((permission) =>
    userPermissions.some(
      (p: any) => p.codename === permission || p.name === permission
    )
  );
}
...
export default defineNuxtRouteMiddleware(async (to) => {
  ...
  try {
    ...
    if (
      ...
      !hasRequiredPermissions(permissions.value, requiredPermissions)
    ) {
      throw createError({
        statusCode: 403,
        statusMessage:
          "Acesso negado: Você não tem as permissões necessárias para acessar esta página",
      });
    }
  } catch (error: any) {
    ...
    if (
      user.value?.is_staff &&
      error.message !== "Acesso negado: Você não tem permissão de administrador"
    ) {
      return;
    }
