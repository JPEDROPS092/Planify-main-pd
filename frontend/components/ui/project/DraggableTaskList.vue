<template>
  <div class="draggable-task-list space-y-4">
    <div class="flex justify-between items-center">
      <h3 class="text-xl font-semibold text-gray-700 dark:text-gray-200">Lista de Tarefas Reordenável</h3>
    ...
    <div v-else-if="tasks.length === 0" class="text-center text-gray-500 dark:text-gray-400 py-6">
    ...
    <div v-else class="bg-white dark:bg-gray-800 rounded-lg p-4 shadow">
      <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
        Arraste e solte as tarefas para reordenar a lista de prioridades. As alterações serão salvas automaticamente.
      </p>
      ...
      >
        <template #item="{element, index}">
          <div class="task-item flex items-center bg-gray-50 dark:bg-gray-700 p-3 rounded-lg shadow-sm hover:shadow-md transition-all">
            <div class="drag-handle cursor-move p-2 mr-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
            ...
            <div class="flex-1">
              <div class="flex justify-between">
                <h4 class="font-medium text-gray-800 dark:text-white">{{ element.name }}</h4>
                <span :class="getStatusClass(element.status)" class="text-xs px-2 py-1 rounded-full">
                  {{ element.status }}
                </span>
              </div>
              ...
              <div class="flex justify-between items-center mt-2 text-xs text-gray-500 dark:text-gray-400">
                ...
                <span v-if="element.due_date">Vencimento: {{ formatDate(element.due_date) }}</span>
              </div>
            </div>
          </div>
        </template>
      ...
      <div v-if="saveSuccess" class="mt-4 p-2 bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 text-sm rounded">
