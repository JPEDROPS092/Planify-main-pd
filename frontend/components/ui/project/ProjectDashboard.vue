      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
        ...
        <div class="space-y-3">
          ...
          <div>
            ...
            >
            <div
              class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5 mt-1"
            >
              <div
            ...
            <p class="text-right text-xs text-gray-500 dark:text-gray-400 mt-1">
              {{ project?.progresso || 0 }}%
            </p>
          </div>
        </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
        ...
        <div class="space-y-3">
          ...
          <div>
            ...
            >
            <p class="font-medium text-gray-800 dark:text-gray-200">
              {{ calculateDuration(project?.data_inicio, project?.data_fim) }}
            </p>
          </div>
          <div v-if="project?.data_fim">
            ...
            <p
              class="font-medium"
              :class="getRemainingDaysClass(project?.data_fim)"
            >
...
async function fetchProjectDetails() {
  ...
  try {
    ...
    await Promise.all([fetchTaskStats(), fetchTeamStats(), fetchRiskStats()]);
  } catch (err: any) {
    error.value = err;
...
async function fetchTeamStats() {
  try {
    const response = await teamService.listEquipes({
      projeto: props.projectId,
    });
...
async function fetchRiskStats() {
  try {
    ...
    riskStats.value = {
      total: risks.length,
      active: risks.filter(
        (r) => r.status !== 'MITIGADO' && r.status !== 'ACEITO'
      ).length,
