<template>
  <div class="gantt-chart">
    <div class="flex justify-between items-center mb-4">
      <h3 class="text-xl font-semibold text-gray-700 dark:text-gray-200">Gráfico de Gantt</h3>
      <div class="flex space-x-2">
        ...
        <Button @click="zoomIn" size="sm" variant="outline" class="flex items-center">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7" />
          </svg>
          Zoom +
        </Button>
        <Button @click="zoomOut" size="sm" variant="outline" class="flex items-center">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <div v-else-if="!tasks.length" class="text-center text-gray-500 dark:text-gray-400 py-6">
    ...
    <div v-else class="gantt-container overflow-x-auto">
      <!-- Cabeçalho com datas -->
      <div class="gantt-header flex border-b border-gray-200 dark:border-gray-700 sticky top-0 bg-white dark:bg-gray-800 z-10">
        <div class="gantt-header-task min-w-[200px] p-2 font-medium text-gray-700 dark:text-gray-300 border-r border-gray-200 dark:border-gray-700">
          Tarefa
        </div>
      ...
      <div class="gantt-body">
        ...
        >
          <div class="gantt-row-task min-w-[200px] p-2 border-r border-gray-200 dark:border-gray-700 flex items-center">
            <div class="w-full">
              <div class="font-medium text-sm text-gray-800 dark:text-gray-200 truncate">{{ task.titulo }}</div>
              <div class="text-xs text-gray-500 dark:text-gray-400 flex items-center mt-1">
                <span class="mr-2">{{ getStatusLabel(task.status) }}</span>
                <span v-if="task.responsavel_nome" class="text-xs text-gray-500 dark:text-gray-400">
          ...
          <div class="gantt-row-timeline flex-1 relative" :style="{ height: '60px' }">
            <!-- Barras de tarefas -->
            <div 
            ...
            >
              <div class="gantt-bar-label px-2 text-xs font-medium whitespace-nowrap overflow-hidden text-ellipsis">
                {{ task.titulo }}
              </div>
            </div>
            <!-- Grade de fundo -->
...
const { error: notifyError, success: notifySuccess, showApiError } = useNotification()
...
async function fetchTasks() {
  ...
  try {
    ...
    tasks.value = response.results.filter(task => 
...
function getTaskBarStyle(task: Tarefa) {
  ...
  const end = task.data_fim ? new Date(task.data_fim) : new Date(task.data_inicio)
