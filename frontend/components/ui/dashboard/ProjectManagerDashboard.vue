<template>
  <div>
    <div class="mb-8">
      ...
      <p class="text-muted-foreground">Visão geral dos projetos e tarefas sob sua responsabilidade</p>
    </div>
    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p class="text-xs text-muted-foreground">Membros ativos nas equipes</p>
        </CardContent>
        <CardFooter>
    ...
    <div class="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-2">
      ...
      <Card>
        ...
        <CardContent>
          <div class="space-y-4">
            <div v-for="task in recentTasks" :key="task.id" class="flex items-start space-x-4">
              ...
              <div class="flex-1">
                ...
                <div class="mt-2 flex items-center justify-between">
                  <p class="text-xs text-muted-foreground">Responsável: {{ task.assignee }}</p>
        ...
        <CardFooter>
          <NuxtLink to="/tarefas" class="text-sm text-muted-foreground hover:text-primary">
    ...
    <Card class="mt-8">
      ...
      <CardContent>
        <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
          <div v-for="status in taskStatusDistribution" :key="status.name" class="flex flex-col items-center justify-center p-4 text-center">
            <div class="flex h-16 w-16 items-center justify-center rounded-full" :class="status.bgClass">
...
import { Briefcase, CheckSquare, Users, AlertTriangle, Flag, CheckCircle2, Clock, XCircle, CircleDashed } from 'lucide-vue-next'
...
const TaskIcon = (props) => {
  ...
  return h(iconMap[props.status] || CircleDashed, { class: getTaskStatusIconClass(props.status) })
}
...
onMounted(async () => {
  try {
    ...
    // Projetos simulados
    projects.value = [
      { id: 1, name: 'Sistema ERP', progress: 75, startDate: '2025-01-15', endDate: '2025-06-30' },
      { id: 2, name: 'Aplicativo Mobile', progress: 45, startDate: '2025-02-10', endDate: '2025-07-15' },
      { id: 3, name: 'Portal do Cliente', progress: 90, startDate: '2025-03-01', endDate: '2025-05-30' },
      { id: 4, name: 'Integração de APIs', progress: 30, startDate: '2025-04-05', endDate: '2025-08-20' },
      { id: 5, name: 'Migração de Dados', progress: 10, startDate: '2025-05-01', endDate: '2025-09-15' }
    ]
    
    // Tarefas recentes simuladas
    recentTasks.value = [
      { id: 1, title: 'Implementar autenticação', description: 'Adicionar sistema de login com múltiplos fatores', status: 'Concluída', assignee: 'João Silva', dueDate: '2025-05-20' },
      { id: 2, title: 'Criar telas de dashboard', description: 'Desenvolver interfaces para diferentes perfis de usuário', status: 'Em Andamento', assignee: 'Maria Santos', dueDate: '2025-05-30' },
      { id: 3, title: 'Testes de integração', description: 'Realizar testes entre módulos do sistema', status: 'Pendente', assignee: 'Pedro Oliveira', dueDate: '2025-06-05' },
      { id: 4, title: 'Correção de bugs', description: 'Resolver problemas reportados pelos usuários', status: 'Atrasada', assignee: 'Ana Costa', dueDate: '2025-05-15' }
    ]
    ...
    taskStatusDistribution.value = [
      { name: 'Concluídas', count: 42, percentage: 35, bgClass: 'bg-green-100 dark:bg-green-900/20', textClass: 'text-green-500 dark:text-green-400' },
      { name: 'Em Andamento', count: 38, percentage: 32, bgClass: 'bg-blue-100 dark:bg-blue-900/20', textClass: 'text-blue-500 dark:text-blue-400' },
      { name: 'Pendentes', count: 27, percentage: 23, bgClass: 'bg-yellow-100 dark:bg-yellow-900/20', textClass: 'text-yellow-500 dark:text-yellow-400' },
      { name: 'Atrasadas', count: 12, percentage: 10, bgClass: 'bg-red-100 dark:bg-red-900/20', textClass: 'text-red-500 dark:text-red-400' }
    ]
    
  } catch (error) {
    console.error('Erro ao carregar dados do dashboard de gerente de projetos:', error)
  }
})
</script>
