...
import { z } from 'zod';
import { useCostService } from '~/services/api/costService';
import { useDocumentService } from '~/services/api/documentService';
import { useNotification } from '~/composables/useNotification';
import { useFormValidation } from '~/composables/useFormValidation';
import { useAuth } from '~/composables/useAuth';
import type { Custo, Documento, User } from '~/services/api/types';
...
async function loadAttachments() {
  ...
  try {
    const response = await documentService.getDocumentosByCusto(props.cost.id);
...
async function uploadCostFiles(custoId: number) {
  ...
  const uploadPromises = filesToUpload.value.map(file => {
    ...
    formData.append('custo', custoId.toString());
  ...
  try {
    ...
    if (custoId) {
      const response = await documentService.getDocumentosByCusto(custoId);
      attachments.value = response.results || [];
    }
  } catch (error) {
...
async function saveCost(formData: any) {
  try {
    let custoId = props.cost?.id;
    let result;
    
    if (custoId) {
      // Atualizar custo existente
      result = await costService.updateCusto(custoId, {
        ...formData,
        id: custoId
      });
      ...
      // Criar novo custo
      result = await costService.createCusto(formData);
      custoId = result.id; // Corrigido: result.id em vez de result.data.id
      notify.success('Custo registrado com sucesso');
    }
    
    // Fazer upload de arquivos se houver
    if (custoId && filesToUpload.value.length > 0) {
      await uploadCostFiles(custoId);
    }
    
    return result; // Corrigido: retornar result em vez de result.data
  } catch (error) {
...
function initForm() {
  ...
  // Preencher o formulário com os dados do custo
  resetForm({
    description: props.cost.descricao || '',
    amount: props.cost.valor || 0,
    date: props.cost.data || new Date().toISOString().split('T')[0],
    category: props.cost.categoria?.id || props.cost.categoria || 'OUTROS', // Suporte para categoria como objeto ou string
    project: props.cost.projeto?.id || props.cost.projeto || props.projectId, // Suporte para projeto como objeto ou ID
  });
...
watch(() => props.projectId, (newProjectId) => {
  updateField('project', newProjectId);
});
