<template>
  <button
    :class="[
      ...
      variant === 'outline' ? 'border border-gray-300 bg-transparent text-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-800' : '',
      size === 'sm' ? 'text-xs' : '',
      size === 'md' ? 'text-sm' : '',
      size === 'lg' ? 'text-base' : '',
      fullWidth ? 'w-full' : '',
...
export default defineComponent({
  ...
  props: {
    variant: {
      type: String,
      default: 'primary',
      validator: (value: string) => ['primary', 'secondary', 'danger', 'success', 'warning', 'outline'].includes(value)
    },
    size: {
      type: String,
      default: 'md',
