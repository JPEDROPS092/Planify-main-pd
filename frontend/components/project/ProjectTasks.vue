<template>
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <h3 class="text-xl font-semibold text-gray-700 dark:text-gray-200">Tarefas do Projeto</h3>
    ...
    <div v-else-if="tasksError" class_name="text-center text-red-500 dark:text-red-400 py-6">
      <p>{{ tasksError.message || 'Erro ao carregar tarefas.' }}</p>
      <Button variant="outline" size="sm" class_name="mt-2" @click="fetchTasks">Tentar Novamente</Button>
    </div>
    <div v-else-if="tasks.length === 0" class_name="text-center text-gray-500 dark:text-gray-400 py-6">
      <p>Nenhuma tarefa encontrada para este projeto.</p>
      <p class_name="text-sm">Crie a primeira tarefa clicando em "Nova Tarefa".</p>
    </div>
    <div v-else class_name="space-y-4">
      ...
      <div v-for="task in tasks" :key="task.id" class_name="bg-white dark:bg-gray-800 p-4 rounded-lg shadow hover:shadow-md transition-shadow">
        <div class_name="flex justify-between items-start">
          <div>
            <h4 class_name="text-lg font-medium text-gray-800 dark:text-white">{{ task.name }}</h4>
            <p class_name="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ task.description }}</p>
            <div class_name="mt-2 flex items-center space-x-3 text-xs text-gray-500 dark:text-gray-400">
              <span>Status: <span :class_name="getStatusClass(task.status)" class_name="font-semibold py-0.5 px-1.5 rounded-full">{{ task.status }}</span></span>
              <span v-if="task.due_date">Vencimento: {{ formatDate(task.due_date) }}</span>
              <span v-if="task.assignee_details">Responsável: {{ task.assignee_details.first_name || task.assignee_details.username }}</span>
            </div>
          </div>
          <div class_name="flex space-x-1">
            <Button variant="ghost" size="icon-sm" @click="openEditTaskModal(task)" title="Editar Tarefa">
            ...
            <Button variant="ghost" size="icon-sm" @click="confirmDeleteTask(task.id)" title="Excluir Tarefa" class_name="text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300">
              <svg xmlns="http://www.w3.org/2000/svg" class_name="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <Modal :is-open="showCreateTaskModal || !!editingTask" @close="closeTaskModal" :title="editingTask ? 'Editar Tarefa' : 'Criar Nova Tarefa'">
      <TaskForm 
...
async function fetchTasks() {
  ...
  try {
    ...
    tasks.value = await Promise.all(taskList.map(async (task: any) => {
...
async function handleTaskFormSubmit(formData: TaskCreate | TaskUpdate) {
  try {
    ...
    showApiError(error, editingTask.value ? 'Falha ao atualizar tarefa' : 'Falha ao criar tarefa')
  }
}
