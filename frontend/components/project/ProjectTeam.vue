<template>
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <h3 class="text-xl font-semibold text-gray-700 dark:text-gray-200">Membros da Equipe</h3>
      <Button @click="showAddMemberModal = true" size="sm" v-if="canManageTeam">
        <svg xmlns="http://www.w3.org/2000/svg" class_name="h-4 w-4 mr-1.5" viewBox="0 0 20 20" fill="currentColor">
    ...
    <div v-if="loadingTeam" class_name="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
      <div v-for="n in 3" :key="n" class_name="bg-white dark:bg-gray-800 p-4 rounded-lg shadow flex items-center space-x-3">
    ...
    <div v-else-if="teamError" class_name="text-center text-red-500 dark:text-red-400 py-6">
      <p>{{ teamError.message || 'Erro ao carregar equipe.' }}</p>
      <Button variant="outline" size="sm" class_name="mt-2" @click="fetchTeamMembers">Tentar Novamente</Button>
    </div>
    <div v-else-if="teamMembers.length === 0" class_name="text-center text-gray-500 dark:text-gray-400 py-6">
      <p>Nenhum membro na equipe deste projeto ainda.</p>
      <p class_name="text-sm" v-if="canManageTeam">Adicione o primeiro membro clicando em "Adicionar Membro".</p>
    </div>
    <div v-else class_name="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      <div v-for="member in teamMembers" :key="member.id" class_name="bg-white dark:bg-gray-800 p-4 rounded-lg shadow hover:shadow-md transition-shadow flex flex-col items-center text-center">
        <!-- Idealmente, teríamos um campo avatar_url no User -->
        <div class_name="w-16 h-16 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-xl font-semibold text-gray-500 dark:text-gray-400 mb-3 uppercase">
          {{ member.first_name?.charAt(0) }}{{ member.last_name?.charAt(0) || member.username?.charAt(0) }}
        </div>
        <h4 class_name="text-md font-medium text-gray-800 dark:text-white">{{ member.first_name }} {{ member.last_name }}</h4>
        <p class_name="text-xs text-gray-500 dark:text-gray-400">@{{ member.username }}</p>
    ...
    <Modal :is-open="showAddMemberModal" @close="closeAddMemberModal" title="Adicionar Membro à Equipe">
      <div class="space-y-4">
        <div>
          <label for="user-select" class_name="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Selecionar Usuário</label>
          <select 
            id="user-select" 
          ...
          >
            <option :value="null" disabled>{{ loadingUsers ? 'Carregando usuários...' : (availableUsers.length === 0 ? 'Nenhum usuário disponível' : 'Selecione um usuário') }}</option>
            <option v-for="user in availableUsers" :key="user.id" :value="user.id">
              {{ user.first_name }} {{ user.last_name }} (@{{ user.username }})
            </option>
          </select>
          <p v-if="addUserError" class_name="text-xs text-red-500 mt-1">{{ addUserError }}</p>
        </div>
        <!-- Poderia adicionar um campo para "Função no Projeto" aqui -->
        <div class_name="flex justify-end space-x-3">
          <Button variant="outline" @click="closeAddMemberModal">Cancelar</Button>
          <Button @click="handleAddMember" :loading="addingMember" :disabled="!selectedUserIdToAdd || addingMember">
            Adicionar Membro
          </Button>
        </div>
      </div>
...
import type { TeamMember } from '~/services/teamService' // Assumindo que TeamMember é a interface para um membro da equipe (pode ser User)
...
const availableUsers = computed(() => {
  return allUsers.value.filter(user => 
    !teamMembers.value.some(member => member.id === user.id)
  )
})
...
async function handleAddMember() {
  ...
  addUserError.value = null
  try {
    await teamService.addMemberToProject(projectId.value, selectedUserIdToAdd.value)
...
async function confirmRemoveMember(memberToRemove: User) {
  ...
  const confirmed = await confirm(
    ...
    { confirmButtonText: 'Remover', cancelButtonText: 'Cancelar', type: 'danger' }
  );

  if (confirmed) {
    try {
      await teamService.removeMemberFromProject(projectId.value, memberToRemove.id)
