<template>
  <div class="sprint-management space-y-6">
    <div class="flex justify-between items-center">
      <h3 class="text-xl font-semibold text-gray-700 dark:text-gray-200">Gerenciamento de Sprints</h3>
    ...
    <div v-else-if="error" class="text-center text-red-500 dark:text-red-400 py-6">
      ...
      <Button variant="outline" size="sm" class="mt-2" @click="fetchSprints">Tentar Novamente</Button>
    </div>
    <div v-else-if="sprints.length === 0" class="text-center text-gray-500 dark:text-gray-400 py-6">
    ...
    <div v-else class="sprint-list space-y-6">
      <div v-for="sprint in sprints" :key="sprint.id" class="sprint-container bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
        <div class="sprint-header flex justify-between items-start mb-4">
          <div>
            <h4 class="font-medium text-gray-800 dark:text-white">{{ sprint.name }}</h4>
          ...
          <div class="flex space-x-2">
            <Button variant="ghost" size="icon-sm" @click="editSprint(sprint)" title="Editar Sprint">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
              </svg>
            </Button>
            <Button variant="ghost" size="icon-sm" @click="confirmDeleteSprint(sprint.id)" title="Excluir Sprint" class="text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div class="sprint-content grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="sprint-tasks bg-white dark:bg-gray-700 p-3 rounded-lg">
            <div class="flex justify-between items-center mb-3 pb-2 border-b border-gray-200 dark:border-gray-600">
              <h5 class="text-sm font-medium text-gray-700 dark:text-gray-300">Tarefas na Sprint</h5>
            ...
            >
              <template #item="{element}">
                <div class="task-card bg-gray-50 dark:bg-gray-800 p-2 rounded shadow-sm hover:shadow-md cursor-move">
                  <div class="task-header flex justify-between items-start">
                    <h6 class="font-medium text-gray-800 dark:text-white text-sm">{{ element.name }}</h6>
                    <span :class="getStatusClass(element.status)" class="text-xs px-1.5 py-0.5 rounded-full">
                      {{ element.status }}
                    </span>
                  </div>
                  <p class="text-xs text-gray-600 dark:text-gray-400 mt-1 line-clamp-1">{{ element.description }}</p>
                </div>
              </template>
              <template #footer v-if="getTasksInSprint(sprint.id).length === 0">
                <div class="text-center text-gray-500 dark:text-gray-400 py-4 text-xs">
          ...
          <div class="available-tasks bg-white dark:bg-gray-700 p-3 rounded-lg">
            <div class="flex justify-between items-center mb-3 pb-2 border-b border-gray-200 dark:border-gray-600">
              <h5 class="text-sm font-medium text-gray-700 dark:text-gray-300">Tarefas Disponíveis</h5>
              <span class="bg-gray-200 text-gray-700 dark:bg-gray-800 dark:text-gray-300 text-xs px-2 py-1 rounded-full">
    ...
    <Modal :is-open="showCreateSprintModal || !!editingSprint" @close="closeSprintModal" :title="editingSprint ? 'Editar Sprint' : 'Criar Nova Sprint'">
      <div class="space-y-4 p-4">
        <div class="form-group">
          <label for="sprint-name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nome da Sprint</label>
          <input 
            id="sprint-name" 
        ...
        <div class="form-group grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label for="start-date" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Data de Início</label>
            <input 
              id="start-date" 
          ...
          <div>
            <label for="end-date" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Data de Término</label>
            <input 
              id="end-date" 
        ...
        <div class="form-group">
          <label for="sprint-goal" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Objetivo da Sprint</label>
          <textarea 
            id="sprint-goal" 
...
const sprintTasksMap = ref<Record<number, Task[]>>({})
...
const isSprintFormValid = computed(() => {
  return sprintForm.value.name.trim() !== '' && 
...
function getStatusClass(status?: string) {
  switch (status?.toLowerCase()) {
    case 'a fazer':
    case 'pendente': return 'bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-gray-300';
    case 'em andamento': return 'bg-blue-200 text-blue-700 dark:bg-blue-900 dark:text-blue-300';
    case 'concluído':
    case 'feito': return 'bg-green-200 text-green-700 dark:bg-green-900 dark:text-green-300';
    case 'bloqueado': return 'bg-yellow-200 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300';
    case 'cancelado': return 'bg-red-200 text-red-700 dark:bg-red-900 dark:text-red-300';
