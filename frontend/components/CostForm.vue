<template>
  <form @submit.prevent="submitForm" class_name="space-y-6">
    <div>
      <label for="cost-description" class_name="block text-sm font-medium text-gray-700 dark:text-gray-300">Descrição do Custo</label>
      <input type="text" id="cost-description" v-model="form.description" required
    ...
    <div class_name="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div>
        <label for="cost-amount" class_name="block text-sm font-medium text-gray-700 dark:text-gray-300">Valor (R$)</label>
        <input type="number" id="cost-amount" v-model.number="form.amount" required step="0.01" min="0"
               class_name="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-gray-700 dark:text-white" />
        <span v-if="errors.amount" class_name="text-xs text-red-500">{{ errors.amount }}</span>
      </div>
      <div>
        <label for="cost-date" class_name="block text-sm font-medium text-gray-700 dark:text-gray-300">Data do Custo</label>
        <input type="date" id="cost-date" v-model="form.date" required
               class_name="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-gray-700 dark:text-white" />
        <span v-if="errors.date" class_name="text-xs text-red-500">{{ errors.date }}</span>
      </div>
    </div>

    <div>
      <label for="cost-category" class_name="block text-sm font-medium text-gray-700 dark:text-gray-300">Categoria</label>
      <select id="cost-category" v-model="form.category" required
      ...
      <span v-if="errors.category" class_name="text-xs text-red-500">{{ errors.category }}</span>
    </div>
...
const costSchema = z.object({
  ...
  category: z.enum(costCategories, { errorMap: () => ({ message: 'Selecione uma categoria válida.' }) }),
  project: z.number(),
