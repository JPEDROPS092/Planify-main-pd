...
async function handleTaskFormSubmit(formData: TarefaCreate | TarefaUpdate) {
  try {
    if (editingTask.value && editingTask.value.id) {
      await taskService.update(editingTask.value.id, formData as TarefaUpdate);
      notifySuccess('Tarefa atualizada com sucesso!');
    } else {
      await taskService.create(formData as TarefaCreate);
