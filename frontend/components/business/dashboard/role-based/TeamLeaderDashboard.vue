      ...
      <CardContent>
        <div class="space-y-6">
          <div v-for="member in teamMembers" :key="member.id" class="space-y-2">
            ...
            >
              <span
                >{{ member.completedTasks }} de
                {{ member.totalTasks }} tarefas</span
              >
        ...
        <CardContent>
          <div class="space-y-4">
            ...
            <div class="mt-6">
              ...
              <div
                v-for="member in taskDistributionByMember"
                :key="member.id"
                class="mb-4"
              >
                <div class="flex items-center justify-between">
                ...
                <div class="mt-1 h-2 w-full rounded-full bg-muted">
                  <div
                    ...
                    :style="{
                      width: `${(member.taskCount / totalTeamTasks) * 100}%`,
                    }"
                  ></div>
                </div>
              </div>
            ...
            >
              ...
              <div class="flex-1">
                ...
                <div class="mt-2 flex items-center justify-between">
                  <p class="text-xs text-muted-foreground">
                    <span v-if="meeting.attendees"
                      >{{ meeting.attendees }} participantes</span
                    >
                  </p>
                  <p class="text-xs text-muted-foreground">
            ...
            <div class="flex items-center justify-between">
              <div class="flex items-center space-x-2">
                <div
                  ...
                  :class="getIssueTypeClass(issue.type)"
                >
                  <IssueIcon
                    :type="issue.type"
                    class="h-4 w-4"
                    :class="getIssueTypeTextClass(issue.type)"
                  />
                </div>
            ...
            <div class="mt-4 flex items-center justify-between">
              <div class="flex items-center space-x-2">
                <Avatar class="h-6 w-6">
                  <AvatarImage
                    v-if="issue.reportedBy.avatar"
                    :src="issue.reportedBy.avatar"
                  />
                  <AvatarFallback>{{
                    getInitials(issue.reportedBy.name)
                  }}</AvatarFallback>
                </Avatar>
                <span class="text-xs text-muted-foreground"
                  >Reportado por {{ issue.reportedBy.name }}</span
                >
              </div>
              <span class="text-xs text-muted-foreground">{{
...
onMounted(async () => {
  try {
    ...
    issues.value = [
      {
        id: 1,
        title: 'Erro na integração com API externa',
        description:
          'A API de pagamentos está retornando erro 500 intermitentemente',
        type: 'bug',
        priority: 'Alta',
        reportedBy: { name: 'Carlos Oliveira', avatar: null },
        reportedDate: '2025-05-26',
      },
      {
        id: 2,
        title: 'Acesso ao ambiente de homologação',
        description:
          'Precisamos de acesso ao ambiente de homologação para testar as novas funcionalidades',
        type: 'impedimento',
        priority: 'Média',
        reportedBy: { name: 'Juliana Costa', avatar: null },
        reportedDate: '2025-05-25',
      },
      {
        id: 3,
        title: 'Dúvida sobre requisitos do módulo financeiro',
        description:
          'Não está claro como deve funcionar o cálculo de juros no sistema',
        type: 'duvida',
        priority: 'Baixa',
        reportedBy: { name: 'Ana Silva', avatar: null },
        reportedDate: '2025-05-24',
      },
    ];
  } catch (error) {
    console.error(
      'Erro ao carregar dados do dashboard de líder de equipe:',
      error
    );
  }
});
