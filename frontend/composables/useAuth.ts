    ...
    try {
      ...
      if (err instanceof ApiError && (err.status === 401 || err.status === 403)) {
        ...
        try {
          ...
          console.log('Refresh bem-sucedido, tentando buscar usuário novamente...');
          ...
          console.error('Falha ao atualizar token durante checkAuth:', refreshErr);
        ...
        console.error('Erro não relacionado a token inválido durante checkAuth, deslogando:', err);
        if (process.client) {
          localStorage.removeItem('auth_token');
          localStorage.removeItem('refresh_token');
        }
    ...
    try {
      ...
      if (err instanceof ApiError) {
        if (err.status === 400 || err.status === 401) {
          error.value = 'Usuário ou senha inválidos. Verifique seus dados e tente novamente.'
        } else if (err.status === 403) {
          if (err.data && err.data.detail && err.data.detail.includes('locked')) {
            error.value = 'Sua conta foi bloqueada devido a múltiplas tentativas de login inválidas. Entre em contato com um administrador.'
          } else {
            error.value = 'Acesso negado. Você não tem permissão para acessar este recurso.'
          }
        } else if (err.status >= 500) {
          error.value = 'Ocorreu um erro no servidor. Por favor, tente novamente mais tarde.'
        } else {
          error.value = err.data?.detail || err.message || 'Ocorreu um erro ao tentar fazer login. Por favor, tente novamente.'
        }
      } else if (err instanceof Error) {
        error.value = err.message
      } else {
        error.value = 'Ocorreu um erro desconhecido ao fazer login. Por favor, tente novamente.'
      }
