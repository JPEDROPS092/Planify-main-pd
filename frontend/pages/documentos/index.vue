  ...
  <NuxtLayout name="dashboard">
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        ...
        <button @click="openNewDocumentModal" class="inline-flex items-center justify-center rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow transition-colors hover:bg-blue-700 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-blue-700">
      ...
      <div v-else-if="error" class="rounded-md bg-red-50 p-4 text-red-700">
        <p>{{ error }}</p>
        <button @click="fetchDocuments" class="mt-2 text-sm font-medium underline">Tentar novamente</button>
      </div>

      <!-- Empty state -->
      <div v-else-if="filteredDocuments.length === 0" class="flex flex-col items-center justify-center rounded-lg border border-dashed border-gray-300 bg-gray-50 p-12 text-center">
        ...
        <p class="mt-1 text-sm text-gray-500">
          {{ documents.length === 0 ? 'Comece fazendo upload do seu primeiro documento.' : 'Nenhum documento corresponde aos filtros aplicados.' }}
        </p>
        <button
          ...
          <table class="w-full border-collapse text-left">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th scope="col" class="px-4 py-3 text-sm font-medium text-gray-500">Tipo</th>
                <th scope="col" class="px-4 py-3 text-sm font-medium text-gray-500">Projeto</th>
                <th scope="col" class="px-4 py-3 text-sm font-medium text-gray-500">Versão</th>
                <th scope="col" class="px-4 py-3 text-sm font-medium text-gray-500">Enviado por</th>
                <th scope="col" class="px-4 py-3 text-sm font-medium text-gray-500">Data</th>
            ...
            <tbody class="divide-y divide-gray-200">
              <tr v-for="doc in filteredDocuments" :key="doc.id" class="hover:bg-gray-50">
                <td class="px-4 py-3 text-sm">
                  ...
                  <div class="mt-1 line-clamp-1 text-xs text-gray-500">{{ doc.descricao }}</div>
                </td>
                <td class="px-4 py-3 text-sm">
                  <span
                    :class="{
      ...
      <div v-if="showDocumentModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
        <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
          <div class="flex items-center justify-between">
            ...
            <button @click="showDocumentModal = false" class="rounded-full p-1 hover:bg-gray-100">
          ...
          <form @submit.prevent="uploadDocument" class="mt-4 space-y-4">
            ...
            <div class="flex justify-end space-x-3 pt-4">
              ...
              >
                <span v-if="uploading" class="mr-2 h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent"></span>
...
const filteredDocuments = computed(() => {
  return documents.value.filter(doc => {
    const matchesSearch = searchQuery.value === '' || 
      ...
      (doc.descricao && doc.descricao.toLowerCase().includes(searchQuery.value.toLowerCase()))
    
    const matchesType = typeFilter.value === '' || doc.tipo === typeFilter.value
    const matchesProject = projectFilter.value === '' || doc.projeto === parseInt(projectFilter.value)
...
const uploadDocument = async () => {
  ...
  try {
    ...
    alert('Não foi possível fazer o upload do documento. Por favor, tente novamente.')
  } finally {
