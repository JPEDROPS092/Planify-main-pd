  ...
  <NuxtLayout name="dashboard">
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Riscos</h1>
        <button @click="openNewRiskModal" class="inline-flex items-center justify-center rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow transition-colors hover:bg-blue-700 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-blue-700 dark:bg-blue-700 dark:hover:bg-blue-600">
      ...
      <div v-if="loading" class="flex flex-col items-center justify-center py-12">
        ...
        <p class="mt-4 text-sm text-gray-500 dark:text-gray-400">Carregando riscos...</p>
      </div>
      <EmptyState 
        ...
        >
          <div class="p-6">
            <div class="flex items-center justify-between">
              <span
                :class="{
                  'bg-yellow-100 text-yellow-800': risk.status === 'IDENTIFICADO',
                  ...
                  'bg-purple-100 text-purple-800': risk.status === 'TRANSFERIDO',
                }"
            ...
            <h3 class="mt-2 text-lg font-semibold text-gray-900 dark:text-white">{{ risk.titulo }}</h3>
            <p class="mt-1 line-clamp-2 text-sm text-gray-500 dark:text-gray-400">{{ risk.descricao }}</p>
            
            <div class="mt-4 grid grid-cols-2 gap-2">
              <div>
                <h4 class="text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Probabilidade</h4>
                <span
                  :class="{
              ...
              <div>
                <h4 class="text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Impacto</h4>
                <span
                  :class="{
            ...
            <div class="mt-4 flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
              <div class="flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4 mr-1 text-gray-400 dark:text-gray-500">
      ...
      <div v-if="showRiskModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
        <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-xl dark:bg-gray-900">
          <div class="flex items-center justify-between">
            <h3 class="text-lg font-medium text-gray-900 dark:text-white">{{ editingRisk ? 'Editar Risco' : 'Novo Risco' }}</h3>
            <button @click="showRiskModal = false" class="rounded-full p-1 hover:bg-gray-100 dark:hover:bg-gray-700">
          ...
          <form @submit.prevent="saveRisk" class="mt-4 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label for="probabilidade" class="block text-sm font-medium text-gray-700 dark:text-gray-400">Probabilidade</label>
                <select
                  id="probabilidade"
              ...
              <div>
                <label for="impacto" class="block text-sm font-medium text-gray-700 dark:text-gray-400">Impacto</label>
                <select
                  id="impacto"
            ...
            <div>
              <label for="plano_mitigacao" class="block text-sm font-medium text-gray-700 dark:text-gray-400">Plano de Mitigação</label>
              <textarea
                id="plano_mitigacao"
...
const filteredRisks = computed(() => {
  return risks.value.filter(risk => {
    ...
    const matchesProbability = probabilityFilter.value === '' || risk.probabilidade === probabilityFilter.value
    const matchesImpact = impactFilter.value === '' || risk.impacto === impactFilter.value
    const matchesStatus = statusFilter.value === '' || risk.status === statusFilter.value
    const matchesProject = projectFilter.value === '' || risk.projeto === parseInt(projectFilter.value)
    
    return matchesSearch && matchesProbability && matchesImpact && matchesStatus && matchesProject
  })
})
