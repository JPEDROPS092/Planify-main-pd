        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
          <div>
            <div class="flex items-center mb-2">
              <Button variant="ghost" size="sm" @click="navigateToProject" class="mr-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                  <path fill-rule="evenodd" d="M9.707 16.707a1 1 0 01-1.414 0l-6-6a1 1 0 010-1.414l6-6a1 1 0 011.414 1.414L5.414 9H17a1 1 0 110 2H5.414l4.293 4.293a1 1 0 010 1.414z" clip-rule="evenodd" />
                </svg>
              </Button>
              <h1 class="text-2xl md:text-3xl font-bold text-gray-800 dark:text-white">{{ project.name }}</h1>
            </div>
            <p class="text-gray-600 dark:text-gray-400">Gráfico Gantt - Visualização de Cronograma</p>
          </div>
...
async function fetchProjectDetails() {
  ...
  try {
    const fetchedProject = await projectService.retrieveProjeto(projectId.value)
