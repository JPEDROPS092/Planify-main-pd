...
const sortBy = ref<"titulo" | "data_termino" | "prioridade" | "status">("data_termino");
...
const {
  ...
  error,
  refetch,
} = useTasksTarefasList(computed(() => ({ 
  page: currentPage.value,
  search: searchQuery.value || undefined,
  status: selectedStatus.value !== "ALL" ? selectedStatus.value : undefined,
  prioridade: selectedPriority.value !== "ALL" ? selectedPriority.value : undefined,
})));
...
const totalPages = computed(() =>
  paginatedTasks.value?.count
    ? Math.ceil(paginatedTasks.value.count / 20)
    : 1
);
...
const taskStats = computed(() => {
  ...
  allTasks.value.forEach(task => {
    switch (task.status) {
      case "A_FAZER": stats.todo++; break;
      case "EM_ANDAMENTO": stats.inProgress++; break;
      case "FEITO": stats.done++; break;
    }
    
    if (task.prioridade === "ALTA") stats.highPriority++;
    
    if (task.data_termino && isAfter(new Date(), new Date(task.data_termino)) && task.status !== "FEITO") {
      stats.overdue++;
    }
  });
...
const getStatusClass = (status: StatusEnum) => {
  const classes = {
    A_FAZER: "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-900/30 dark:text-amber-300 dark:border-amber-700",
    EM_ANDAMENTO: "bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-700",
    FEITO: "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-300 dark:border-emerald-700",
  };
  return classes[status] || "bg-gray-50 text-gray-700 border-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600";
};
...
const getPriorityIcon = (priority: PrioridadeEnum) => {
  const icons = {
    ALTA: "lucide:alert-triangle",
    MEDIA: "lucide:minus",
...
const getTaskUrgencyClass = (task: TarefaList) => {
  ...
  if (task.data_termino && isBefore(new Date(task.data_termino), addDays(new Date(), 3))) {
    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
          <div class="flex items-center justify-between">
            ...
            <div class="p-3 bg-blue-100 dark:bg-blue-900/30 rounded-lg">
              <Icon icon="lucide:list-checks" class="w-6 h-6 text-blue-600 dark:text-blue-400" />
            </div>
          </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm font-medium text-gray-600 dark:text-gray-400">A Fazer</p>
              <p class="text-2xl font-bold text-amber-600 dark:text-amber-400">{{ taskStats.todo }}</p>
            </div>
            <div class="p-3 bg-amber-100 dark:bg-amber-900/30 rounded-lg">
              <Icon icon="lucide:circle" class="w-6 h-6 text-amber-600 dark:text-amber-400" />
            </div>
          </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
          <div class="flex items-center justify-between">
            ...
            <div class="p-3 bg-blue-100 dark:bg-blue-900/30 rounded-lg">
              <Icon icon="lucide:clock" class="w-6 h-6 text-blue-600 dark:text-blue-400" />
            </div>
          </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{{ taskStats.done }}</p>
            </div>
            <div class="p-3 bg-emerald-100 dark:bg-emerald-900/30 rounded-lg">
              <Icon icon="lucide:check-circle" class="w-6 h-6 text-emerald-600 dark:text-emerald-400" />
            </div>
          </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-red-600 dark:text-red-400">{{ taskStats.overdue }}</p>
            </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700 mb-8">
        <div class="flex flex-col lg:flex-row gap-4">
          ...
          <div class="flex-1">
            <div class="relative">
              <Icon icon="lucide:search" class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />
              <input
                v-model="searchQuery"
                type="text"
      ...
      <div v-if="isLoading" class="flex items-center justify-center py-20">
        <div class="text-center">
          <Icon icon="svg-spinners:180-ring-with-bg" class="w-12 h-12 mx-auto text-blue-600 mb-4" />
      ...
      <div v-else-if="error" class="text-center py-20">
        <div class="bg-red-50 dark:bg-red-900/30 rounded-xl p-8 max-w-md mx-auto">
          <Icon icon="lucide:alert-circle" class="w-12 h-12 mx-auto text-red-600 dark:text-red-400 mb-4" />
          <h3 class="text-lg font-semibold text-red-900 dark:text-red-100 mb-2">Erro ao carregar tarefas</h3>
          <p class="text-red-700 dark:text-red-300 mb-4">Não foi possível carregar as tarefas. Tente novamente.</p>
          <button
            @click="refetch"
      ...
      <div v-else-if="tasks.length === 0" class="text-center py-20">
        <div class="bg-gray-50 dark:bg-gray-800 rounded-xl p-8 max-w-md mx-auto">
          <Icon icon="lucide:clipboard-list" class="w-16 h-16 mx-auto text-gray-400 mb-4" />
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">Nenhuma tarefa encontrada</h3>
          <p class="text-gray-600 dark:text-gray-400 mb-6">Comece criando sua primeira tarefa para organizar seu trabalho.</p>
          <button
            @click="handleCreateTask"
      ...
      <div v-else>
            ...
            <div class="p-6">
              ...
              <div class="flex items-start justify-between mb-4">
                <div class="flex-1">
                  ...
                  <div class="flex items-center gap-2 mb-3">
                    <span :class="getStatusClass(task.status)" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border">
                      <Icon :icon="getStatusIcon(task.status)" class="w-3 h-3" />
                      {{ getStatusLabel(task.status) }}
                    </span>
                    <span :class="getPriorityClass(task.prioridade)" class="inline-flex items-center gap-1 text-xs font-medium">
                      <Icon :icon="getPriorityIcon(task.prioridade)" class="w-3 h-3" />
              ...
              <div class="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                ...
                <div v-if="task.data_termino" class="flex items-center gap-2">
                  <Icon icon="lucide:calendar" class="w-4 h-4" :class="isTaskOverdue(task) ? 'text-red-500' : ''" />
                  <span :class="isTaskOverdue(task) ? 'text-red-600 dark:text-red-400 font-medium' : ''">
                    {{ formatDate(task.data_termino) }}
                    <span class="text-xs ml-1">({{ formatDateRelative(task.data_termino) }})</span>
                  </span>
                </div>
              </div>
              
              <!-- Overdue Warning -->
              <div v-if="isTaskOverdue(task)" class="mt-3 p-2 bg-red-50 dark:bg-red-900/30 rounded-lg">
                <div class="flex items-center gap-2 text-red-700 dark:text-red-300 text-xs">
        ...
        <div v-if="totalPages > 1" class="mt-8 flex items-center justify-center">
          <nav class="flex items-center gap-2">
            ...
            <span class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg">
