  ...
  <NuxtLayout name="dashboard">
    <div class="space-y-6">
      ...
      <div v-else-if="error" class="rounded-md bg-red-50 p-4 text-red-700">
        <p>{{ error }}</p>
        <button @click="fetchTasks" class="mt-2 text-sm font-medium underline">
          Tentar novamente
        </button>
      </div>
...
const filteredTasks = computed(() => {
  return tasks.value.filter((task) => {
    const matchesSearch =
      ...
      (task.descricao &&
        task.descricao.toLowerCase().includes(searchQuery.value.toLowerCase()));
    ...
    const matchesProject =
      projectFilter.value === '' ||
      task.projeto === parseInt(projectFilter.value);
...
const fetchTasks = async () => {
  ...
  try {
    ...
    error.value =
      'Não foi possível carregar as tarefas. Por favor, tente novamente.';
  } finally {
    loading.value = false;
  }
};
...
watch(
  [currentPage, searchQuery, statusFilter, priorityFilter, projectFilter],
  () => {
    fetchTasks();
  }
);
