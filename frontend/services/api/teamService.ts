  ...
  const createTeam = async (teamData) => {
    return withLoading(async () => {
      try {
        ...
        if (
          !teamData.members ||
          !teamData.members.some((member) => member.user_id === user.value?.id)
        ) {
          teamData.members = [
            ...(teamData.members || []),
  ...
  const addTeamMember = async (teamId, memberData) => {
    return withLoading(async () => {
      try {
        ...
        if (
          team.members.some((member) => member.user_id === memberData.user_id)
        ) {
 ...
 */
export async function partialUpdateEquipe(
  id: number,
  payload: Partial<EquipeRequest>
): Promise<Equipe> {
  return getApi()<Equipe>(`/api/teams/equipes/${id}/`, {
    method: 'PATCH',
    body: payload,
...
export async function adicionarMembroEquipe(
  id: number,
  payload: { usuario: number; papel: string }
 ...
 * @param payload Dados do membro
 */
export async function atualizarPapelMembroEquipe(
  id: number,
  payload: { membro_id: number; papel: string }
): Promise<MembroEquipe> {
  return getApi()<MembroEquipe>(
 ...
 */
export async function partialUpdateMembro(
  id: number,
  payload: Partial<MembroEquipeRequest>
): Promise<MembroEquipe> {
  return getApi()<MembroEquipe>(`/api/teams/membros/${id}/`, {
    method: 'PATCH',
    body: payload,
...
export async function createPermissao(
  payload: PermissaoEquipeRequest
): Promise<PermissaoEquipe> {
 ...
 */
export async function partialUpdatePermissao(
  id: number,
  payload: Partial<PermissaoEquipeRequest>
): Promise<PermissaoEquipe> {
  return getApi()<PermissaoEquipe>(`/api/teams/permissoes/${id}/`, {
    method: 'PATCH',
    body: payload,
