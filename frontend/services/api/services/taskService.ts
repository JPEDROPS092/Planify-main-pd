...
export async function listTarefas(params?: {
  ...
  if (params?.responsavel)
    queryParams.append('responsavel', params.responsavel.toString());
 ...
 */
export async function partialUpdateTarefa(
  id: number,
 ...
 */
export async function associarSprintTarefa(
  id: number,
  payload: { sprint: number }
): Promise<Tarefa> {
 ...
 */
export async function registrarHorasTarefa(
  id: number,
  payload: { horas: number; descricao?: string }
): Promise<Tarefa> {
 ...
 */
export async function createAtribuicao(payload: {
  tarefa: number;
  usuario: number;
}): Promise<any> {
 ...
 */
export async function partialUpdateComentario(
  id: number,
  payload: { texto?: string }
): Promise<any> {
  return getApi()<any>(`/api/tasks/comentarios/${id}/`, {
...
export const useTaskService = () => {
  ...
  const partialUpdateTask = async (
    id: number,
    taskData: Partial<TarefaRequest>
  ) => {
    isLoading.value = true;
    error.value = null;
