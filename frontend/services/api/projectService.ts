...
export const useProjectService = () => {
  ...
  const partialUpdateProject = async (
    id: number,
    projectData: Partial<ProjetoRequest>
  ) => {
    isLoading.value = true;
    error.value = null;
  ...
  const archiveProject = async (id: number, arquivado: boolean) => {
    ...
    try {
      ...
      error.value =
        err.message ||
        `Erro ao ${arquivado ? 'arquivar' : 'desarquivar'} projeto ${id}`;
      throw err;
    } finally {
      isLoading.value = false;
    }
...
// ... (rest of the code remains the same)
  ...
  if (params?.arquivado !== undefined)
    queryParams.append('arquivado', params.arquivado.toString());
 ...
 */
export async function partialUpdateProjeto(
  id: number,
  payload: Partial<ProjetoRequest>
): Promise<Projeto> {
  return getApi()<Projeto>(`/api/projects/projetos/${id}/`, {
    method: 'PATCH',
    body: payload,
 ...
 */
export async function archiveProjeto(
  id: number,
  payload: { arquivado: boolean }
): Promise<Projeto> {
