...
export const useTaskService = () => {
  ...
  const fetchTasks = async (projectId = null) => {
    ...
    try {
      if (projectId) {
        const response = await tasksApi.listTarefas({ projeto: projectId })
        tasks.value = response.results
      } else {
        const response = await tasksApi.listTarefas()
        tasks.value = response.results
      }
      return tasks.value
    } catch (err) {
  ...
  const getTask = async (taskId) => {
    ...
    try {
      const task = await tasksApi.retrieveTarefa(taskId)
      return task
    } catch (err) {
      error.value = handleApiError(err)
      throw err
  ...
  const createTask = async (taskData) => {
    return withLoading(async () => {
      try {
        ...
        const newTask = await tasksApi.createTarefa(taskData)
  ...
  const updateTask = async (taskId, taskData) => {
    return withLoading(async () => {
      try {
        const updatedTask = await tasksApi.updateTarefa(taskId, taskData)
  ...
  const deleteTask = async (taskId) => {
    return withLoading(async () => {
      try {
        ...
        await tasksApi.destroyTarefa(taskId)
  ...
  const updateTaskStatus = async (taskId, status) => {
    return withLoading(async () => {
      try {
        const updatedTask = await tasksApi.atualizarStatusTarefa(taskId, { status })
  ...
  const addTaskComment = async (taskId, commentData) => {
    return withLoading(async () => {
      try {
        const updatedTask = await tasksApi.adicionarComentarioTarefa(taskId, commentData)
