...
export const useProjectService = () => {
  ...
  const getProject = async (projectId) => {
    ...
    try {
      const project = await projectsApi.retrieveProject(projectId)
      return project
    } catch (err) {
      error.value = handleApiError(err)
      throw err
  ...
  const deleteProject = async (projectId) => {
    return withLoading(async () => {
      try {
        ...
        await projectsApi.destroyProject(projectId)
  ...
  const addProjectMember = async (projectId, memberData) => {
    return withLoading(async () => {
      try {
        const updatedProject = await projectsApi.addProjectMember(projectId, memberData)
