...
class TaskService:
        ...
        with transaction.atomic():
            ...
            for user in assignees:
                AtribuicaoTarefa.objects.create(
                    tarefa=tarefa, usuario=user, atribuido_por=created_by
                )
    ...
    def _validate_status_transition(current_status, new_status):
        ...
        """
        # Define allowed transitions - using correct status values from model
        allowed_transitions = {
            "A_FAZER": ["EM_ANDAMENTO"],
            "EM_ANDAMENTO": ["A_FAZER", "FEITO"],
            "FEITO": ["EM_ANDAMENTO"],  # Allow reopening
        }
    ...
    def assign_task(tarefa, usuario, assigned_by):
        ...
        return AtribuicaoTarefa.objects.create(
            tarefa=tarefa, usuario=usuario, atribuido_por=assigned_by
        )
    ...
    def add_comment(tarefa, autor, conteudo, parent_comment=None):
        ...
        return ComentarioTarefa.objects.create(
            tarefa=tarefa, autor=autor, conteudo=conteudo, comentario_pai=parent_comment
        )
    ...
    def get_task_metrics(tarefa):
        ...
        return {
            ...
            "days_since_created": (
                timezone.now().date() - tarefa.criado_em.date()
            ).days,
            "estimated_completion": TaskService.estimate_task_completion(tarefa),
            "is_overdue": (
                (
                    tarefa.data_termino_prevista
                    and timezone.now().date() > tarefa.data_termino_prevista
                    and tarefa.status != "concluida"
                )
                if hasattr(tarefa, "data_termino_prevista")
                else False
            ),
        }
