...
class CategoriaAdmin(admin.ModelAdmin):
    ...
    list_filter = get_all_fields(Categoria)
    search_fields = ['nome', 'descricao']

@admin.register(Custo)
class CustoAdmin(admin.ModelAdmin):
    list_display = get_all_fields(Custo)
    list_filter = get_all_fields(Custo)
...
class OrcamentoProjetoAdmin(admin.ModelAdmin):
    list_display = get_all_fields(OrcamentoProjeto)
    list_filter = get_all_fields(OrcamentoProjeto)
    search_fields = ['projeto__name']

@admin.register(OrcamentoTarefa)
class OrcamentoTarefaAdmin(admin.ModelAdmin):
    list_display = get_all_fields(OrcamentoTarefa)
    list_filter = get_all_fields(OrcamentoTarefa)
    search_fields = ['tarefa__titulo']

@admin.register(Alerta)
class AlertaAdmin(admin.ModelAdmin):
    list_display = get_all_fields(Alerta)
    list_filter = get_all_fields(Alerta)
